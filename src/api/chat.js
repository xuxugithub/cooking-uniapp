...
// 获取会话列表（按更新时间倒序）
export const getConversationList = (options = {}) => {
  return get('/api/conversation/list', {}, options)
}

// 创建新会话
export const createConversation = (title = '小助手会话', options = {}) => {
  return post('/api/conversation/create', { title }, options)
}

// 获取会话消息
export const getConversationMessages = (conversationId, options = {}) => {
  return get(`/api/conversation/${conversationId}/messages`, {}, options)
}

// 保存消息到会话
export const saveConversationMessage = (data, options = {}) => {
  return post('/api/conversation/message', data, options)
}

// 同步对话
export const syncChat = (question, conversationId, options = {}) => {
  return post('/api/chat/sync', { question, conversationId }, options)
}
...
const repairUtf8Mojibake = (text) => {
  ...
  try {
    // 将每个字符的 charCode 当作原始字节重新用 UTF-8 解码
...
const decodeChunkData = (data, decoder) => {
  if (!data) return ''
  if (typeof data === 'string') return repairUtf8Mojibake(data)
...
const parseSseTextFallback = (rawText) => {
  ...
  blocks.forEach((block) => {
    ...
    const dataLines = trimmed
      .split('\n')
      .filter((line) => line.replace(/^\uFEFF/, '').startsWith('data:'))
      .map((line) => line.replace(/^\uFEFF/, '').slice(5).trimStart())
...
const extractTextPayload = (payload, decoder) => {
  if (!payload) return ''
  if (typeof payload === 'string') return repairUtf8Mojibake(payload)
  if (payload instanceof ArrayBuffer || ArrayBuffer.isView(payload)) {
    return decodeChunkData(payload, decoder)
  }

  if (typeof payload === 'object') {
    if (typeof payload.data === 'string') return repairUtf8Mojibake(payload.data)
    if (payload.data instanceof ArrayBuffer || ArrayBuffer.isView(payload.data)) {
      return decodeChunkData(payload.data, decoder)
    }
    if (typeof payload.content === 'string') return repairUtf8Mojibake(payload.content)
  }

  return ''
}

export const streamChat = (question, conversationId, handlers = {}, options = {}) => {
  ...
  const promise = new Promise((resolve, reject) => {
    ...
    requestTask = uni.request({
      ...
      success: (res) => {
        ...
        // In some runtimes success may fire before all chunks arrive.
        // Delay finalization and let chunk callbacks drive completion.
        if (!chunkReceived) {
      ...
      fail: (error) => {
        if (settled) return
        rejectOnce(new Error(error?.errMsg || 'network error'))
      }
    })
